  ...
  describe FormItemDecorator, :odk, :reset_factory_sequences, database_cleaner: :truncate do
    ...
    describe "xpath methods" do
      ...
      describe "xpath_to" do
        context "with prepend_current false" do
          it "returns . when same question" do
            expect(q2.xpath_to(q2)).to eq(".")
          end
        ...
        context "with prepend_current true" do
          it "returns current() when same question" do
            expect(q2.xpath_to(q2, prepend_current: true)).to eq("current()")
          end

          it "returns the relative xpath when staying on same level within group" do
            expect(q3.xpath_to(q2, prepend_current: true)).to eq("current()/../qing#{q2.id}")
          end

          it "returns the relative xpath when going up within group" do
            expect(q4.xpath_to(q2, prepend_current: true)).to eq("current()/../../qing#{q2.id}")
          end
