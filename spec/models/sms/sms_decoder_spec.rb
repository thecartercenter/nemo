...
describe Sms::Decoder, :sms do
  ...
  describe "authentication" do
    context "with SMS Authentication enabled" do
      ...
      it "should work with the correct code provided" do
        ...
        expect_decoding(body: "#{auth_code} #{form.code} 1.17", answers: [17])
      end

      it "should raise an error with an incorrect code provided" do
        auth_code = "n000"
        expect_decoding_fail(body: "#{auth_code} #{form.code} 1.17", error: "user_not_found")
      end

      it "should raise an error if no code is provided" do
        expect_decoding_fail(body: "#{form.code} 1.17", error: "user_not_found")
      end
      ...
      it "should lock account after 3 failed attempts" do
        ...
        expect_decoding_fail(body: "#{form.code} 1.17", error: "account_locked")
      end
  ...
  describe "form lookup" do
    it "submitting to unpublished form should produce appropriate error" do
      ...
      @form.unpublish!
      expect_decoding_fail(body: "#{@form.code} 1.15", error: "form_not_published")
    end

    it "submitting to non-existent form should produce appropriate error", :investigate do
      expect_decoding_fail(body: "abc 1.15", error: "form_not_found")
    end
    ...
    it "submitting to outdated form should produce appropriate error" do
      ...
      expect_decoding_fail(body: "#{old_version_code} 1.15", error: "form_version_outdated")
    end
    ...
    it "submitting to non-smsable form should produce appropriate error" do
      ...
      expect_decoding_fail(body: "#{@form.code} 1.15", error: "form_not_smsable")
    end
    ...
    it "submitting to form without permission should produce appropriate error" do
      ...
      expect_decoding_fail(body: "#{@form.code} 1.15", user: other_user, error: "form_not_permitted")
    end

    it "form code should be case insensitive" do
      create_form(questions: %w(integer))
      expect_decoding(body: "#{@form.code.upcase} 1.15", answers: [15])
    end
  end
  ...
  describe "user lookup" do
    it "submitting from phone number without plus sign should work" do
      ...
      expect_decoding(body: "#{@form.code} 1.15", from: phone, answers: [15])
    end

    it "submitting from unrecognized phone number should error" do
      create_form(questions: %w(integer))
      expect_decoding_fail(body: "#{@form.code} 1.15", from: "+12737272722", error: "user_not_found")
    end

    it "submitting from phone number with letters should raise special error" do
      create_form(questions: %w(integer))
      expect_decoding_fail(body: "#{@form.code} 1.15", from: "DEALS", error: "automated_sender")
    end

    it "submitting from shortcode should raise special error" do
      create_form(questions: %w(integer))
      expect_decoding_fail(body: "#{@form.code} 1.15", from: "123456", error: "automated_sender")
    end
    ...
    it "submitting from second phone number should work" do
      ...
      expect_decoding(body: "#{@form.code} 1.15", from: second_phone, answers: [15])
    end
  end
  ...
  describe "mission lookup and checking" do
    ...
    context "with mission pre-specified and form in mission" do
      it "succeeds" do
        expect_decoding(body: "#{form.code} 1.15", answers: [15], mission: form.mission)
      end
    end

    context "with mission pre-specified and form from other mission" do
      it "fails with form not found error" do
        expect_decoding_fail(body: "#{form.code} 1.15", mission: create(:mission), error: "form_not_found")
      end
    end

    context "with no mission pre-specified" do
      it "succeeds and sets correct mission on message" do
        expect_decoding(body: "#{form.code} 1.15", answers: [15], mission: nil)
  ...
  describe "answer parsing" do
    it "form with text question should work" do
      ...
      expect_decoding(body: "#{@form.code} 1.weather is very cold 2.234", answers: ["weather is very cold", 234])
    end
    ...
    it "form with single question should work" do
      create_form(questions: %w(integer))
      expect_decoding(body: "#{@form.code} 1.15", answers: [15])
    end

    it "submitting garbage should fail" do
      expect_decoding_fail(body: "lasjdalfksldjal", error: "invalid_form_code")
    end

    it "form with invalid integer should error" do
      create_form(questions: %w(integer))
      expect_decoding_fail(body: "#{@form.code} 1.1d", error: "answer_not_integer", rank: 1, value: "1d")
    end

    it "form with invalid question rank should error" do
      create_form(questions: %w(integer))
      expect_decoding_fail(body: "#{@form.code} 1.15 2.8", error: "question_doesnt_exist", rank: 2)
    end

    it "form with duplicate answers for the same question should error" do
      create_form(questions: %w(integer integer))
      expect_decoding_fail(body: "#{@form.code} 1.15 2.8 2.9", error: "duplicate_answer", rank: 2)
    end

    it "spaces after decimal points should not cause error" do
      create_form(questions: %w(integer integer))
      expect_decoding(body: "#{@form.code} 1. 15 2. 8", answers: [15, 8])
    end

    it "select_one question should work" do
      create_form(questions: %w(integer select_one))
      expect_decoding(body: "#{@form.code} 1.15 2.b", answers: [15, "Banana"])
    end

    it "select_one question with numeric option should error" do
      create_form(questions: %w(integer select_one))
      expect_decoding_fail(body: "#{@form.code} 1.15 2.6", error: "answer_not_valid_option", rank: 2, value: "6")
    end

    it "select_one question with non-existent option should error" do
      create_form(questions: %w(integer select_one))
      expect_decoding_fail(body: "#{@form.code} 1.15 2.h", error: "answer_not_valid_option", rank: 2, value: "h")
    end

    it "select_one question treated as text should work" do
      create_form(questions: %w(integer select_one_as_text_for_sms))
      expect_decoding(body: "#{@form.code} 1.15 2.banana", answers: [15, "Banana"])
    end
    ...
    it "select_one question treated as text should work if option name has spaces" do
      ...
      expect_decoding(body: "#{@form.code} 1.15 2.Elder berry 3.99", answers: [15, "Elder Berry", 99])
    end

    it "select_one question treated as text should not work with normal encoding" do
      create_form(questions: %w(integer select_one_as_text_for_sms))
      expect_decoding_fail(body: "#{@form.code} 1.15 2.b", error: "answer_not_valid_option", rank: 2, value: "b")
    end

    it "select_one question treated as text should not work if no match" do
      create_form(questions: %w(integer select_one_as_text_for_sms))
      expect_decoding_fail(body: "#{@form.code} 1.15 2.Peach", error: "answer_not_valid_option", rank: 2, value: "Peach")
    end
    ...
    it "select_one question treated as text should work for multilevel option set" do
      ...
      expect_decoding(body: "#{@form.code} 1.15 2.tulip", answers: [15, ["Plant", "Tulip"]])
    end

    it "select_one question treated as text should work for multilevel option set for non-leaf option" do
      create_form(questions: %w(integer multilevel_select_one_as_text_for_sms), default_option_names: true)
      expect_decoding(body: "#{@form.code} 1.15 2.Plant", answers: [15, ["Plant", "NIL"]])
    end
    ...
    it "select_one question with appendix and invalid option should not work" do
      ...
      expect_decoding_fail(body: "#{@form.code} 1.15 2.no", error: "answer_not_valid_option")
    end

    it "select_one question with appendix should work" do
      create_form(questions: %w(integer select_one_with_appendix_for_sms))
      expect_decoding(body: "#{@form.code} 1.15 2.2", answers: [15, "Banana"])
    end
    ...
    it "select_multiple question should work" do
      ...
      expect_decoding(body: "#{@form.code} 1.15 2.bd", answers: [15, %w(Banana Durian)])
    end
    ...
    it "select_multiple question with appendix should work" do
      ...
      expect_decoding(body: "#{@form.code} 1.15 2.2,4", answers: [15, %w(Banana Durian)])
    end
    ...
    it "select_multiple questions with large option set should work" do
      ...
      expect_decoding(body: "#{@form.code} 1.15 2.11,20,1a", answers: [15, %w(1 36 10)])
    end
    ...
    it "select_multiple questions with large option set and more than 4 selections should work" do
      create_form(questions: %w(integer large_select_multiple))
      expect_decoding(body: "#{@form.code} 1.15 2.11,12,13,14,15,20,1a", answers: [15, %w(1 2 3 4 5 36 10)])
    end

    it "counter question should work" do
      create_form(questions: %w(counter))
      expect_decoding(body: "#{@form.code} 1.7", answers: [7])
    end

    it "decimal question should work" do
      create_form(questions: %w(decimal))
      expect_decoding(body: "#{@form.code} 1.1.15", answers: [1.15])
    end
    ...
    it "decimal question with invalid answer should error" do
      create_form(questions: %w(decimal))
      expect_decoding_fail(body: "#{@form.code} 1.15.2.2", error: "answer_not_decimal", rank: 1, value: "15.2.2")
    end

    it "text question at beginning of message should work" do
      create_form(questions: %w(text integer))
      expect_decoding(body: "#{@form.code} 1.foo bar 2.15", answers: ["foo bar", 15])
    end

    it "long_text question at beginning of message should work" do
      create_form(questions: %w(long_text integer))
      expect_decoding(body: "#{@form.code} 1.foo bar that is very long 2.15", answers: ["foo bar that is very long", 15])
    end

    it "text question in middle of message should work" do
      create_form(questions: %w(select_one text integer))
      expect_decoding(body: "#{@form.code} 1.a 2.foo bar 3.15", answers: ["Apple", "foo bar", 15])
    end
    ...
    it "text question at end of message should work" do
      ...
      expect_decoding(body: "#{@form.code} 1.a 2.15 3.foo bar", answers: ["Apple", 15, "foo bar"])
    end
    ...
    it "date question should work" do
      ...
      expect_decoding(body: "#{@form.code} 1.4 2.20120229", answers: [4, Date.new(2012, 2, 29)])

      # check other formats
      expect_decoding(body: "#{@form.code} 1.4 2.2012/02/29", answers: [4, Date.new(2012, 2, 29)])
      expect_decoding(body: "#{@form.code} 1.4 2.2012.02.29", answers: [4, Date.new(2012, 2, 29)])
      expect_decoding(body: "#{@form.code} 1.4 2.2012.2.29", answers: [4, Date.new(2012, 2, 29)])
    end

    it "date question with invalid date should error" do
      create_form(questions: %w(integer date))
      expect_decoding_fail(body: "#{@form.code} 1.4 2.20120230", error: "answer_not_date", value: "20120230")
    end

    it "date question with too short date should error" do
      create_form(questions: %w(integer date))
      expect_decoding_fail(body: "#{@form.code} 1.4 2.2012230", error: "answer_not_date", value: "2012230")
    end

    it "date question with junk should error" do
      create_form(questions: %w(integer date))
      expect_decoding_fail(body: "#{@form.code} 1.4 2.foobarbaz", error: "answer_not_date", value: "foobarbaz")
    end

    it "time question should work" do
      create_form(questions: %w(integer time))
      expect_decoding(body: "#{@form.code} 1.4 2.1230", answers: [4, Time.parse("2000-01-01 12:30 UTC")])
      ...
      # check other formats
      expect_decoding(body: "#{@form.code} 1.4 2.12:30", answers: [4, Time.parse("2000-01-01 12:30 UTC")])
      expect_decoding(body: "#{@form.code} 1.4 2.12:30pm", answers: [4, Time.parse("2000-01-01 12:30 UTC")])
      expect_decoding(body: "#{@form.code} 1.4 2.12:45PM", answers: [4, Time.parse("2000-01-01 12:45 UTC")])
      expect_decoding(body: "#{@form.code} 1.4 2.12.30pm", answers: [4, Time.parse("2000-01-01 12:30 UTC")])
      expect_decoding(body: "#{@form.code} 1.4 2.130", answers: [4, Time.parse("2000-01-01 1:30 UTC")])
      expect_decoding(body: "#{@form.code} 1.4 2.0130", answers: [4, Time.parse("2000-01-01 1:30 UTC")])
      expect_decoding(body: "#{@form.code} 1.4 2.1:30", answers: [4, Time.parse("2000-01-01 1:30 UTC")])
      expect_decoding(body: "#{@form.code} 1.4 2.1:30am", answers: [4, Time.parse("2000-01-01 1:30 UTC")])
      expect_decoding(body: "#{@form.code} 1.4 2.1:30pm", answers: [4, Time.parse("2000-01-01 13:30 UTC")])
    end

    it "invalid times should error" do
      create_form(questions: %w(integer time))
      ["12:300", "25:00", "00000", "12", "abc"].each do |str|
        expect_decoding_fail(body: "#{@form.code} 1.4 2.#{str}", error: "answer_not_time", value: str)
      end
    end
    ...
    it "datetime question should work" do
      ...
      # check other formats
      expect_decoding(body: "#{@form.code} 1.4 2.20120229 230", answers: [4, Time.zone.parse("2012-02-29 2:30")])
      expect_decoding(body: "#{@form.code} 1.4 2.20120229 12:30pm", answers: [4, Time.zone.parse("2012-02-29 12:30")])
      expect_decoding(body: "#{@form.code} 1.4 2.20120229 1:30", answers: [4, Time.zone.parse("2012-02-29 1:30")])
      expect_decoding(body: "#{@form.code} 1.4 2.2012/02/29 12:30pm", answers: [4, Time.zone.parse("2012-02-29 12:30")])
      expect_decoding(body: "#{@form.code} 1.4 2.2012-02-29 12:30pm", answers: [4, Time.zone.parse("2012-02-29 12:30")])
      expect_decoding(body: "#{@form.code} 1.4 2.2012-11-1 12:30pm", answers: [4, Time.zone.parse("2012-11-01 12:30")])
      expect_decoding(body: "#{@form.code} 1.4 2.201211011230", answers: [4, Time.zone.parse("2012-11-01 12:30")])
      expect_decoding(body: "#{@form.code} 1.4 2.201211010230", answers: [4, Time.zone.parse("2012-11-01 2:30")])
    end

    it "invalid datetimes should error" do
      create_form(questions: %w(integer datetime))
      ["2012121212300", "mar 1 2012 2:30", "201212", "891015 12pm", "2-2-2012 5pm"].each do |str|
        expect_decoding_fail(body: "#{@form.code} 1.4 2.#{str}", error: "answer_not_datetime", value: str)
      end
    end
  ...
  describe "duplicate checking" do
    it "duplicate sent within timeframe should error" do
      create_form(questions: %w(integer))
      expect_decoding(body: "#{@form.code} 1.4", answers: [4])
      Timecop.travel(Sms::Decoder::DUPLICATE_WINDOW - 1.minute) do
        expect_decoding_fail(body: "#{@form.code} 1.4", error: "duplicate_submission")
      end
    end
  ...
  def expect_decoding_fail(options)
    error = nil
    begin
      expect_decoding(options.merge(expecting_fail: true))
