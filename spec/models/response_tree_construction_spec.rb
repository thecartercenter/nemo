...
describe Response do
  ...
  context "tree is valid" do
    ...
    it "builds a correct tree" do
      ...
      expect_children(root_node.c[0], %w[Answer], [form.c[0].c[0].id], [1])
    end

    it "saves the correct tree" do
      root_node = response.build_root_node(type: "AnswerGroup", form_item: form.root_group, new_rank: 0)
      group = root_node.children.build(type: "AnswerGroup", form_item: form.c[0], new_rank: 0)
      group.children.build(type: "Answer", form_item: form.c[0].c[0], new_rank: 0, value: int_answer)
      root_node.children.build(type: "Answer", form_item: form.c[1], new_rank: 1, value: "A")
      response.save!
