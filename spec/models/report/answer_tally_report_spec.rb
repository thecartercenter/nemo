...
describe Report::AnswerTallyReport do
  ...
  context "with multilevel option set" do
    before do
      ...
      create(:response, form: @form, answer_values: [%w[Animal Cat]])
      create(:response, form: @form, answer_values: [%w[Animal Dog]])
      create(:response, form: @form, answer_values: [["Animal"]])
      create(:response, form: @form, answer_values: [%w[Plant Oak]])
  ...
  describe "results" do
    it "counts of yes and no for all yes no questions" do
      ...
      report = create_report("AnswerTally", option_set: yes_no, filter: %(form: "#{forms[0].name}"))
      expect(report).to have_data_grid(%w[Yes No TTL],
        %w[q1 6 4 10],
        %w[q2 7 3 10],
        %w[q3 8 2 10],
