...
# tests for the singleton case, where there is only one subset in the collection,
# are currently in SummaryCollectionSingleTest
...
describe "summary collection with multiple subsets" do
  ...
  it "collections with select_multiple questions should have correct summaries" do
    prepare_form_and_collection("select_multiple", "select_one",
      "a" => [["red"], %w[red green], []], "b" => [%w[blue red], %w[blue green]])
  ...
  it "the disaggregation question should not be included in the report output" do
    ...
    # there should only be one summary (integer type) in each subset.
    # the select_one question should not be included.
  ...
  def prepare_form(analyze_type, dissag_type, answers_by_dissag_value, options = {})
    ...
    answers = answers_by_dissag_value.flat_map { |dissag_value, values| values.map { |v| [v, dissag_value] } }
  ...
  def items_for_disagg_value(val, item_attrib)
    subsets_by_disagg_value[val].summaries.detect { |s| s.questioning.rank == 1 }
      .items.map { |i| i.send(item_attrib) }
  end
