...
describe Odk::SubmissionParser do
  ...
  context "simple form" do
    ...
    # let(:submission!) { Odk::SubmissionParser.new(response: response, files: files) }
    # let(:answer_tree) { response.root_node }
    ...
    it "should produce a simple tree from a form with three children" do
      ...
      Odk::SubmissionParser.new(response: response, files: files)
  ...
  # TODO merge in helper! Accepts a fixture filename and form provided by a spec, and creates xml mimicking odk
