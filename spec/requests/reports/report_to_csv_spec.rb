...
describe "report CSV output", :csv do
  ...
  it "should work for grid-type report" do
    ...
    expect(response.body).to match_user_facing_csv(%("",Cat,Dog\r\n#{form.questions[0].name},2,3\r\n))
  end
  ...
  it "should properly format long text" do
    ...
    get("/en/m/#{form.mission.compact_name}/reports/#{report.id}.csv")
    expect(response).to be_successful
    expect(response.body).to match_user_facing_csv(
      %(#{qs[0].name},#{qs[1].name}\r\nFoo,\"Some **long**\r\n\r\n1. text\r\n2. stuff&stuff\"\r\n)
    )
  end
  ...
  it "should use option value if present" do
    ...
    get("/en/m/#{form.mission.compact_name}/reports/#{report.id}.csv")
    expect(response).to be_successful
    expect(response.body).to match_user_facing_csv("#{form.questions[0].name}\r\n123\r\nDog\r\n")
  end
end
