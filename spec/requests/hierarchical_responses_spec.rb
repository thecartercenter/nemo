...
describe "responses", type: :request do
  ...
  describe "create" do
    it "should work" do
      post(hierarchical_responses_path(mode: "m", mission_name: get_mission.compact_name), response: response_attrs)
      resp = Response.first
      expect(response).to redirect_to hierarchical_responses_path
  ...
  describe "update" do
    ...
    it "should work" do
      put(hierarchical_response_path(resp), response: response_attrs.merge(
        answers_attributes: {
          "2" => {
      ...
      expect(response).to redirect_to(hierarchical_responses_path(mission_name: get_mission.compact_name))
  ...
  describe "csv", :csv do
    ...
    it "should produce valid CSV" do
      get_s(hierarchical_responses_path(mode: "m", mission_name: get_mission.compact_name, format: :csv))
