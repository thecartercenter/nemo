...
describe "response search", js: true do
  ...
  context "with more complex data" do
    ...
    context "search filters" do
      ...
      # TODO: replace react-dates
      xscenario "submit date searches" do
      ...
      scenario "form-based search navigation" do
        new_search_for(%(form-id:#{form.id}))
        expect(page).to have_content(codes[0])
        expect(page).to have_content(codes[1])
