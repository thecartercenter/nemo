...
(function (ns, klass) {
  ...
  klass.match_lists = function (a, b, callback) {
    const build_hash = function (spec) {
      const h = {};
      for (let i = 0; i < spec.list.length; i++) {
        if (spec.comparator == null) h[spec.list[i]] = spec.list[i];
        else h[spec.comparator(spec.list[i])] = spec.list[i];
      }
      return h;
    };
    ...
    for (var key in ah) callback(ah[key], null);

    // b-only's: loop over bh's remaining elements. these had no match in a.
    for (var key in bh) callback(null, bh[key]);
  };
