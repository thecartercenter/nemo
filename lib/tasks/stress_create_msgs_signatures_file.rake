...
namespace :stress do
  ...
  task :create_msgs_signatures_file, [:auth_token, :incoming_token,
    :mission_name, :base_url, :number, :quantity] => [:environment] do |t, args|

    args.with_defaults(auth_token: '', incoming_token: '',
      mission_name: 'missionone', base_url: 'http://localhost:8443',
      number: '+553598765432', quantity: 10)
