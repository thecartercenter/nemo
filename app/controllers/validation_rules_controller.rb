...
class ValidationRulesController < ApplicationController
  ...
  before_action :set_validation_rule, only: %i[show edit update destroy toggle]
  ...
  def toggle
    ...
    redirect_to(validation_rules_path,
      notice: "Validation rule #{@validation_rule.is_active? ? 'activated' : 'deactivated'} successfully.")
  end
  ...
  def validation_rule_params
    params.require(:validation_rule).permit(
      ...
      conditions: %i[operator field value] # Update these keys as appropriate to your application
    )
  end
end
