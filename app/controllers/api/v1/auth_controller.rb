...
module Api
  module V1
    class AuthController < BaseController
      skip_before_action :authenticate_api_user!, only: [:login, :register, :forgot_password, :reset_password]
      ...
      def change_password
        if current_user.valid_password?(params[:current_password])
          if current_user.update(password: params[:new_password], password_confirmation: params[:new_password_confirmation])
            ...
            render json: error_response('Failed to change password', current_user.errors), status: :unprocessable_entity
          end
        else
