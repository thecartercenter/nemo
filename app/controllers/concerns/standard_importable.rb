...
module StandardImportable
  ...
  def import_standard
    ...
    params[:objs_to_import].each do |id|
      begin
        ...
        error_count += 1
        logger.debug("ERROR #{$ERROR_INFO} IMPORTING #{model_class.try(:name)}"\
          " ##{id} TO MISSION #{current_mission.try(:name)}")
  ...
  def load_importable_objs
    return unless !admin_mode? && can?(:import_standard, model_class)
