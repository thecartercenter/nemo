# a collection of SummarySubsets
...
class Report::SummaryCollection
  ...
  # builds a summary collection with the given questionings
  def initialize(questionings)
    ...
    summaries = grouped.keys.map{|g| grouped[g].empty? ? nil : send("summaries_for_#{g}_questionings", grouped[g])}.compact.flatten

    # make a summary_collection
    @subsets = [Report::SummarySubset.new(:disaggregation_value => :all, :summaries => summaries)]
  end
  ...
  private
    ...
    def summaries_for_stat_questionings(stat_qs)
      # do the query
      res = run_stat_query(stat_qs)
      ...
      summaries = res.each(:as => :hash).map do |row|
        ...
        # build summary
        Report::QuestionSummary.new(:questioning => qings_by_id[row['qing_id']], :display_type => :structured, :headers => headers, :items => items, :null_count => row['null_count'])
      end
    ...
    def summaries_for_select_questionings(select_qs)
    ...
    def summaries_for_date_questionings(date_qs)
    ...
    def summaries_for_raw_questionings(raw_qs)
