...
class Sms::Adapters::Adapter
  ...
  def prepare_message_for_delivery(message)
    ...
    raise Sms::Errors::Error, "message body is empty" if message.body.blank?
    raise Sms::Errors::Error, "message has no recipients" if message.recipient_numbers.all?(&:blank?)
  ...
  def send_request(uri, method = :get, payload = {})
    ...
    begin
      ...
      raise Sms::Errors::Error, "error contacting #{service_name} (timeout)"
    rescue
      raise Sms::Errors::Error, "error contacting #{service_name} (#{$!.class.name}: #{$!.to_s})"
    end
    ...
    if response.is_a?(Net::HTTPSuccess)
      ...
      raise Sms::Errors::Error, "error contacting #{service_name} (#{response.class.name})"
    end
  end
end
