...
const Cls = (ELMO.Views.FormItemsDraggableListView = class FormItemsDraggableListView extends ELMO.Views.ApplicationView {
  ...
  check_condition_order(placeholder, item) {
    ...
    for (let c of Array.from(item.find('.refd-qing'))) {
    ...
    for (let id of Array.from(child_ids.get())) {
      for (let refd_qing of Array.from(this.$(`.refd-qing[data-ref-id=${id}]`))) { // Loop over all matching refd_qings
  ...
  compare_ranks(a, b) {
    ...
    if (ar.length === br.length) { return 0; } else if (ar.length > br.length) { return 1; } else { return -1; }
  }
});
Cls.initClass();
