...
ELMO.Views.ResponseFormView = class ResponseFormView extends ELMO.Views.ApplicationView {
  ...
  initialize(params) {
    ...
    $('#response_user_id').select2({ ajax: (new ELMO.Utils.Select2OptionBuilder()).ajax(params.submitter_url, 'possible_users', 'name') });
    $('#response_reviewer_id').select2({ ajax: (new ELMO.Utils.Select2OptionBuilder()).ajax(params.reviewer_url, 'possible_users', 'name') });
  ...
  showLocationPicker(e) {
    e.preventDefault();
    const field = $(e.target).closest('.widget').find('input[type=text]');
