...
class ELMO.Views.ResponseFormView extends ELMO.Views.ApplicationView
  ...
  initialize: (params) ->
    ...
    @$('#response_user_id').select2({ ajax: select2AjaxParams(params.submitter_url, 'possible_users') })
    @$('#response_reviewer_id').select2({ ajax: select2AjaxParams(params.reviewer_url, 'possible_users') })
