...
(function (ns, klass) {
  ...
  klass.prototype.build = function () {
    ...
    this.tally_type.change(() => { _this.broadcast_change('tally_type'); });
    ...
    this.question_labels.change(() => { _this.broadcast_change('question_labels'); });
    ...
    this.question_order.change(() => { _this.broadcast_change('question_order'); });

    // build group by tag chooser
    this.group_by_tag = $('#group_by_tag');
    this.group_by_tag.change(() => { _this.broadcast_change('group_by_tag'); });

    // build disaggregation chooser
    this.disagg_question_chooser = new ELMO.Report.DisaggQuestionSelector(this.menus.question);
    this.disagg_question_chooser.field.change(() => { _this.broadcast_change('disagg_question_id'); });

    // setup an event handler for the disaggregate checkbox
    this.cont.find('#disaggregate').change((e) => {
      ...
      if ($(e.target).is(':checked')) self.disagg_question_chooser.select_first_geographic();
    ...
    this.text_responses.change(() => { _this.broadcast_change('text_responses'); });
  ...
  klass.prototype.update = function (report) {
    ...
    const is_tally = this.report.attribs.type == 'Report::TallyReport';
    let show;
