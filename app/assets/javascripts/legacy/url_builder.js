...
(function(ns, klass) {
  ...
  klass.prototype.strip_scope = function(path) {
    ...
    if (this.mode == 'mission')
      mode_mission = 'm/' + this.mission_name;
    ...
    if (path == '' || path.match(new RegExp('^\/([a-z]{2}(\/' + mode_mission + ')?(\/)?)?$')))
      return '/';
    // else fix the "/en/foo" or "/en/m/mission/foo" variants
    else
      return path.replace(new RegExp('^\/[a-z]{2}(\/' + mode_mission + ')?\/(.+)'), function(m, $1, $2){ return '/' + $2; });
  }
