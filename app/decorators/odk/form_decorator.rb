module Odk
  class FormDecorator < BaseDecorator
    ...
    def needs_manifest?
      ...
      @needs_manifest ||= option_sets.any?(&:multilevel?) || has_audio_prompt_content?
    end

    private

    def has_audio_prompt_content?
      smsable_questionings.values.any? { |qing| qing.audio_prompt_content_type }
    end
  end
end
