...
module AnswersHelper
  ...
  def media_link(object, show_delete: false)
    if object.nil?
      tag.div("[#{t('common.none')}]", class: "no-value")
    else
      tag.div(class: "media-thumbnail") do
        concat(link_to(image_tag(thumb_path(object)), media_path(object), target: "_blank", rel: "noopener"))

        concat(tag.div(class: "links") do
          concat(link_to(tag.i("", class: "fa fa-download"),
            media_path(object, dl: "1"), class: "download"))

          if show_delete
            concat(link_to(tag.i("", class: "fa fa-trash-o"),
              "#",
              class: "delete",
