module ResponsesHelper
  def responses_index_fields
    ...
    if params[:controller] == 'dashboard'
      ...
      fields = %w(id form_id user_id) + key_question_hashes(2) + %w(incomplete created_at age reviewed duplicate actions)
    end
  end
  ...
  def format_responses_field(resp, field)
    ...
    if field.is_a?(Hash)
      ...
      when "duplicate" then resp.duplicate? ? duplicate_notice(resp) : ""
