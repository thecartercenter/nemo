...
class Sms::Adapters::SmsAdapterTest < ActiveSupport::TestCase
  ...
  test "delivering a message with one recipient should work" do
    each_adapter do |adapter|
      assert_equal(true, adapter.deliver(Sms::Message.create(:to => %w(+15556667777), :body => "foo")))
    end
  end
  ...
  test "delivering an invalid message should raise an error" do
    each_adapter do |adapter|
      # no recips
      assert_raise(Sms::Error){adapter.deliver(Sms::Message.create(:to => nil, :body => "foo"))}
    
      # no body
      assert_raise(Sms::Error){adapter.deliver(Sms::Message.create(:to => %w(+15556667777), :body => ""))}
    end
  end
