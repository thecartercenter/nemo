...
class SmsControllerTest < ActionController::TestCase
  ...
  test "reply should be in correct language" do
    ...
    assert_sms_response(:mission => m, :incoming => "#{form_code} 1.15 2.b", :outgoing => /permission.+soumettre.+#{form_code}/i)
    
    # add the user to the mission
    @user.assignments.create(:mission => m, :active => true, :role => Role.highest)
    
    # try again -- should get merci (need different answers else treated as duplciate)
    assert_sms_response(:mission => m, :incoming => "#{form_code} 1.15 2.c", :outgoing => /#{form_code}.+merci/i)
  end
