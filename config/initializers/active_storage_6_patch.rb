...
# Copied from https://github.com/Azure/azure-storage-ruby/issues/166#issuecomment-637696565
# Used in NEMO as a patch for Rails 6.0 before we can upgrade
# to 6.1 which has real support for the azure-storage-blob gem.
      ...
      instrument(:upload, key: key, checksum: checksum) do
        handle_errors do
          if disposition && filename
            content_disposition = content_disposition_with(filename: filename, type: disposition)
          end

          client.create_block_blob(
            container,
            key,
            IO.try_convert(io) || io,
            content_md5: checksum, content_type: content_type, content_disposition: content_disposition
          )
        end
      end
    end
