  ...
  def up
    transaction do
      ...
      OptionSet.where(standard_id: nil).each do |set|
        ...
        # Create root node.
        root = OptionNode.create!({option_set_id: set.id}.merge(%w[is_standard mission_id].map_hash do |a|
          set.send(a)
        end))
        ...
        OptionSet.where(standard_id: set.id).each do |copy|
          copy_root = OptionNode.create!({option_set_id: copy.id,
                                          standard_id: root.id}.merge(%w[is_standard mission_id].map_hash do |a|
                                                                        set.send(a)
                                                                      end))
  ...
  def create_nodes(params)
    ...
    oings.each do |oing|
      ...
      exec_query("SELECT * FROM optionings WHERE standard_id = '#{oing['id']}'").each do |copy|
        copy_node = OptionNode.create!({standard_id: node.id}.merge(%w[option_set_id option_id mission_id is_standard
                                                                       rank].map_hash do |a|
          copy[a]
        end))
        unless (parent_copy = params[:parent_copies][copy["mission_id"]])
          raise "Parent copy for mission #{copy['mission_id']} not found"
        end
