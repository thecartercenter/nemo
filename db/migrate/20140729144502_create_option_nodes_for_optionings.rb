...
class OptionSet < ApplicationRecord; end
class OptionNode < ApplicationRecord; has_ancestry; end
...
class CreateOptionNodesForOptionings < ActiveRecord::Migration[4.2]
  ...
  def create_nodes(params)
    ...
    oings.each do |oing|
      ...
      node.update!(parent: params[:parent_node])
      ...
      exec_query("SELECT * FROM optionings WHERE standard_id = '#{oing['id']}'").each do |copy|
        ...
        copy_node.update!(parent: parent_copy)
